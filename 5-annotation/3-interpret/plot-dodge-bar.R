...
create_dodged_bar_plot <- function(data, x_col, fill_col, var_colors, DBname, fillname, xname, plotTitle, ylim) {
  ...
  ggplot_plot <- ggplot(total_db, aes(x = !!x_col_sym, y = n, fill = !!fill_col_sym,
                                      text = paste("n:", scales::comma(n), ""))) +
    geom_bar(position = "dodge", stat = "identity", width = 0.5, colour = "black", alpha = 0.6) +
    ...
    scale_y_continuous(label=comma, limits = c(0,ylim)) +
    labs(fill = fillname) +
    theme_bw() +
