...
create_stacked_bar_plot <- function(data, x_col, fill_col, var_colors, DBname, fillname, xname, plotTitle) {
...
# create_stacked_bar_plot <- function(data, var_colors, DBname, fillname, xname, plotTitle) {
#
#   # Create the stacked bar plot
#   stack_total_db <- ggplot(data, aes(x = VARIANT.CLASS, y = Percent, fill = ZYG)) +
#     geom_bar(stat = "identity", width = 0.5, colour = "black", alpha = 0.6) +
#     geom_text(aes(label = paste0(sprintf("%1.1f", Percent), "%")),
#               position = position_stack(vjust = 0.5),
#               colour = "black",
