...
# 4-oop-label-cell-line.py 

# this script takes the output of vep for snvs and indels and 
#  adds
...
# %%
indel = pd.read_csv('1_clean_vep_output-indel-cad12-v2.csv', low_memory=False)
snv = pd.read_csv('1_clean_vep_output-snp-cad12-v2.csv', low_memory=False)
print(indel.shape)
print(snv.shape)

# %%
cellLine = pd.concat([indel, snv], axis=0)

# %%

# create a list of unique 'Uploaded_variation' in both dataframes
cad_variations = cellLine['Uploaded_variation'].unique()

# create a separate list for variations that are exclusively in cad12
cad_exclusive_variations = list(set(cad_variations) - set(cad_variations))
...
common_variations = list(set(cad_variations) & set(cad_variations))

print( len(cad_exclusive_variations), len(cad_exclusive_variations), len(common_variations)) 
...
cad12['MAF'] = np.select([cad12['MAX.AF'] < 0.01, (cad12['MAX.AF'] >= 0.01) & (cad12['MAX.AF'] < 0.1), cad12['MAX.AF'] >= 0.1], ['Rare (<1%)', 'Less common (<10%)', 'Common (>=10%)'], default='')

cad63['MAF'] = np.select([cad63['MAX.AF'] < 0.01, (cad63['MAX.AF'] >= 0.01) & (cad63['MAX.AF'] < 0.1), cad63['MAX.AF'] >= 0.1], ['Rare (<1%)', 'Less common (<10%)', 'Common (>=10%)'], default='')
# %%


# convert the R code into python
cad12['MAF2'] = np.select([cad12['MAX.AF'] < 0.05, cad12['MAX.AF'] >= 0.05], ['Rare (<5%)', 'Common (>=5%)'], default='')

cad63['MAF2'] = np.select([cad63['MAX.AF'] < 0.05, cad63['MAX.AF'] >= 0.05], ['Rare (<5%)', 'Common (>=5%)'], default='')
...
# %%
cad12[cad12['ClinVar'] == 'PATHO']
crosstabit(cad12, 'IND', 'ClinVar', True)


# %%
cad63[cad63['ClinVar'] == 'PATHO']
